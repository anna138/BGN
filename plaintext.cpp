...
Plaintext * NewUnbalancedPlaintext(PublicKey *pk, float m)
{
    ...
    if(fmod(mFloat, 1.0) != 0.0) {
        ...
        rationalize(floor(mFloat), pk->FPScaleBase, pk->FPPrecision, &numerator,
                    &scaleFactor);
        mpz_t mInt;
        ...
        coeffs = unbalancedEncode(mInt, pk->PolyBase, degreeTable,
                                  degreeSumTable, &degree);
    ...
    coeffs = unbalancedEncode(mInt, pk->PolyBase, degreeTable, degreeSumTable,
                              &degree);
...
Plaintext * NewPlaintext(PublicKey * pk, mpf_t m)
{
    ...
    if(fmod(mFloat, 1.0) != 0.0) {
        ...
        rationalize(mFloat - floor(mFloat), pk->FPScaleBase, pk->FPPrecision,
        ...
        coeffs = balancedEncode(mInt, pk->PolyBase, degreeTable, degreeSumTable,
                                &degree);
        constructPlaintext(pt, pk, coeffs, degree, scaleFactor);
        return pt;
    }
...
int64_t * unbalancedEncode(mpz_t target, int base, mpz_t * degrees,
                           mpz_t * sumDegrees, int * encodeInt)
{
...
int64_t * balancedEncode(mpz_t target, int base, mpz_t * degrees,
                         mpz_t * sumDegrees, int * encodeInt)
{
    mpz_t zero;
    mpz_init(zero);
...
void rationalize(double x, int base, double precision, int64_t * numb,
                 int * pew)
{
    int factor = floor(x);
...
mpf_t * PolyEval(Plaintext * p)
{
    ...
    if(p->ScaleFactor != 0) {
        int64_t scale = pow((int64_t) p->Pk->FPScaleBase,
                            (int64_t) p->ScaleFactor);
