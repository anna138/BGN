               ...
               SecretKey * sk)
{
    ...
    constructPublicKey(pk, &pairing, G1, P, Q, N, T, polyBase, fpScaleBase,
                       fpPrecision, deterministic);
...
Ciphertext * AInv(Ciphertext *ct, PublicKey * pk)
{
    ...
    for(i = degree - 1; i >= 0; i--) {
        element_set(result[i],
                    *ESubElements(*encryptZero(pk), ct->Coefficients[i], pk));
    }
...
element_t* AInvElementL2(element_t el, PublicKey * pk)
{
    ...
    element_init_same_as(
            el,
            *ESubL2Elements(*ToDeterministicL2Element(*encryptZero(pk), pk), el,
                            pk));
    element_init_same_as(*temp, el);
    return temp;
}
...
Ciphertext * aInvL2(Ciphertext * ct, PublicKey * pk)
{
    ...
    for(i = degree - 1; i >= 0; i--) {
        element_set(
                result[i],
                *ESubL2Elements(*encryptZeroL2(pk), ct->Coefficients[i], pk));
    }
...
Plaintext * decryptL2(Ciphertext * ct, PublicKey * pk, SecretKey * sk)
{
    ...
    for(i = 0; i < ct->Degree; i++) {
        plaintextCoeffs[i] = (int64_t) mpz_get_d(
                *DecodeSign(
                        *DecryptElementL2(ct->Coefficients[i], pk, false, sk),
                        pk));
    }
    Plaintext * pt = (Plaintext *) malloc(sizeof(Plaintext));
                    ...
                    PublicKey *pk)
{
    ...
    for(i = degree - 1; i >= 0; i--) {
        ...
        element_set(
                result[i],
                *EAddL2Elements(ct1->Coefficients[i], ct2->Coefficients[i],
                                pk));
    }
    Ciphertext * ct = (Ciphertext*) malloc(sizeof(Ciphertext));
    constructCiphertext(ct, result, degree, ct1->ScaleFactor, ct1->L2);
...
Ciphertext * eMultC(Ciphertext * ct, mpf_t constant, PublicKey *pk)
{
    ...
    constructCiphertext(product, result, degree,
                        ct->ScaleFactor + poly->ScaleFactor, ct->L2);
